...
#define DECLARE_VEC(type)                                                                    \
  ...
  const type vec_##type##_fold(Vec_##type *vec, const type init,                             \
  ...
  const bool vec_##type##_remove(Vec_##type *vec, const int index)                           \
  {                                                                                          \
    if (index >= 0 && index < vec->len)                                                      \
    {                                                                                        \
      memcpy(vec->ptr + index, vec->ptr + index + 1, (vec->len - index - 1) * sizeof(type)); \
      vec->len = vec->len - 1;                                                               \
      return true;                                                                           \
    }                                                                                        \
    else                                                                                     \
