...
#define DECLARE_VEC(type)                                                                               \
  ...
  {                                                                                                     \
    if (index >= 0 && index < vec->len)                                                                 \
    {                                                                                                   \
      memmove(vec->ptr + index, vec->ptr + index + 1, (vec->len - index - 1) * sizeof(type));           \
      vec->len = vec->len - 1;                                                                          \
      return true;                                                                                      \
    }                                                                                                   \
    else                                                                                                \
  ...
  void vec_##type##_clear(Vec_##type *const vec)                                                        \
  {                                                                                                     \
    vec->len = 0;                                                                                       \
  }                                                                                                     \
  DECLARE_VEC_APPLY_FUNCTION_SIGNATURE(type)                                                            \
  bool vec_##type##_apply_if_exist(Vec_##type *vec, const int index, vec_##type##_apply_function_sig f) \
  {                                                                                                     \
